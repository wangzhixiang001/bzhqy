...
var vm = new Vue({
    ...
    methods: {
        ...
        onload: function() {
            ...
            postAjax('init',"",function(data){
                ...
                if(data.code==0){
                    ...
                    localStorage.setItem("login_account_23_start", _this.Account);
                    _this.hashfun("contId03");
                }else if(data.code ==400){
                    _this.selectList = data.data.departs;
                    _this.slots[0].values = data.data.departs;
                    _this.Account =localStorage.getItem("login_account_23_start");
                    if(_this.Account){
                        _this.LoginBtn();
                    }else {
                        _this.reg_auth();
        ...
        reg_auth:function(){
            ...
            var param ={path:path};
            if(code){
                param ={code: code,path:path};
            }
            postAjax( "qyauth",param ,  //备注
                function (data) {
                    data = eval('('+data+')');
                    if(data.data.number){
                        _this.Account=data.data.number;
